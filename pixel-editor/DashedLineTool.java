...
public class DashedLineTool implements ToolInterface
{
    private Easel easel;
    ...
    private ArrayList<Pixel> removedPixels = new ArrayList<>();
    ...
    public DashedLineTool(Easel easel) { this.easel = easel; }
    ...
    /**Converts a regular line to a dashed line by removing pixels from the list pixelsToChange */
    public void makeDashedLine() {
        removePixels(easel.getPenSize());
        pixelsToChange.removeAll(removedPixels);
        removedPixels.clear();
    }
    /**Populates the list of pixels with all the pixels to be removed from the line to create a dashed line
     * @param pensize current pen size determins line thickness and so, number of pixels to be removed
    */
    public void removePixels(int penSize) {
        int gap = penSize*2;
        int counter=0;
